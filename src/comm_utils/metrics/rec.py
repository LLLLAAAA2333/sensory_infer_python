...
def top_1_accuracy_score(label: np.ndarray, pred: np.ndarray):
    ...
    label = label.squeeze()
    assert pred.shape[0] == label.shape[0], "pred and label are not matching!"
    re = np.sum(np.argmax(pred, axis = 1) == label) / len(label)
    return re


def top_1_accuracy_score_torch(label: torch.Tensor, pred: torch.Tensor):
    """ the number of unique label used in top_k_accuracy_score must equal the second shape of pred, but this func doesn't meet the constraint. """
    label = label.squeeze()
    assert pred.shape[0] == label.shape[0], "pred and label are not matching!"
    re = torch.sum(torch.argmax(pred, dim = 1) == label) / len(label)
    return re
