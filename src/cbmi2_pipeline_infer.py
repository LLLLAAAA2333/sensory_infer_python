...
from src.cbmi2_iter import infer_one_batch
from src.cbmi2_inference import Treeformer_End2End, VolumeMemoryBuffer, draw_volume_result

if __name__ == '__main__':
    # Initialize parameters
    parser = argparse.ArgumentParser(description = "CBMI 2.0 pipeline")
    ...
    parser.add_argument('--config', type = str, default = "src/configs/inference/zm9644_m_c.json", help = "")
    ...
    for stream in tqdm(split_processing_streams(processing_stacks, max_mats_one_stream = args.volume_window), colour = "#FF1493"):
        ...
        if not args.only_preprocessing:
            ...
            ts.extend([results[name][-1][2] for name in results.keys() if results[name][-1] is not None])
            ab_num_neurons.extend([results[name][-1][3] for name in results.keys() if results[name][-1] is not None])
            is_warnings.extend([results[name][-1][4] for name in results.keys() if results[name][-1] is not None])
            df = pd.DataFrame({"regions": num_regions, "neurons": num_neurons, "ts": ts, "ab_num_neurons": ab_num_neurons, "is_warnings": is_warnings})
