...
def load_datapath(folder_list):
    ...
    for folder in folder_list:
        ...
        file_path.sort(key=lambda path: os.path.basename(path)[-7:-4])
...
def volume_alignment(file_list, save_path, shiftrange):
    """
...
def volume_stack_and_create_h5(gcamp_file_list, ref_file_list, zephir_folder_path):
    ...
    Output: HDF5 file containing stacked volumes
    """
    ...
    try:
        with h5py.File(h5file_path, 'a') as f:
            if '/x' not in f:
                ...
                f.create_dataset('/id', shape=(0,), maxshape=max_shape, dtype='uint8')
            ...
            for volume_index, neuron_pt_tuple in enumerate(neuron_all):
                ...
                f['/worldline_id'][-n:] = np.arange(1, n+1)
...
def generate_volume_video(sorted_volume_list, synthetic_volume_save_path, neuron_pt_tuple, shift_list):
    ...
    vis.Plot3DResult(lambda ptr: load_volume_image_from_npy(image_name_dict[ptr], z_ratio = 10.0/3.0,source_min = 120, source_max = 200,red_pseudo_color = False),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--process-stack-root', type = str, default = "/home/data4/WJH/olfactory_data/20241116_wen0065/w3/green", help = '')
    parser.add_argument('--save-preprocess-result-root', type = str, default = '')
    parser.add_argument('--json-store-root', type = str, default = "/home/data4/WJH/result/20241116_wen0065/w3")
    parser.add_argument('--zephir-support-root', type = str, default="/home/wenlab-user/RongWei/olfactory/code_v1.10/src/zephir")
    ...
    aligned_volumes_mip, shift_list = volume_alignment(sorted_volume_list, synthetic_volume_save_path, shiftrange=(101, 11))
    # aligned_volumes_mip = np.load(os.path.join(synthetic_volume_save_path, 'aligned_volumes_mip.npy'))
    # shift_list =  np.load(os.path.join(synthetic_volume_save_path, 'shift_list.npy'))
    ...
    create_worldlines(np.arange(1, len(neuron_pt_tuple)+1), zephir_save_path)
    ...
    # 处理 data.h5 文件，取前 10 行
    data_datasets = ['data']
    extract_subset(zephir_save_path + '/data.h5', data_datasets, zephir_save_path + '/data_sub.h5', num_rows=10)

    # 处理 annotations.h5 文件
    annotations_datasets = ['provenance', 't_idx', 'worldline_id', 'x', 'y', 'z']
    extract_subset(zephir_save_path + '/annotations.h5', annotations_datasets, zephir_save_path + '/subset_annotations.h5', num_rows=num_neuron*10-1)
    print("新的 HDF5 文件 'data_sub.h5'、'subset_annotations.h5' 已生成")   
