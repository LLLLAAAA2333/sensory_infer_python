...
def calc_weighted_score(brightness, score):
    return brightness / 10000. * score # approximate calculation
...
def depict_neuron_by_tuple(region_pt_tuple: torch.Tensor, neuron_dict: Dict, rec_len_pt: int = 8, rec_pt_mode: str = "p"):
    ...
    for idx, (nid, idxes) in enumerate(neuron_dict.items()):
        ...
        if "i" in rec_pt_mode:
            ...
            area = neurons[:, 3] * neurons[:, 4]
            area_ratio = area / torch.sum(neurons[:, 3] * neurons[:, 4])
            _constant = torch.stack([area_ratio, area], dim=1)
            neuron_pt_tuple[idx, 6:8] = torch.sum(neurons[:, 6:8] * _constant, dim = 0)
...
class Aligner_PCA(nn.Module):
    ...
    def __init__(self, dtype = torch.float32, device = torch.device("cpu"), is_aligned: bool = True):
        """
...
class Treeformer_End2End(nn.Module):
    def __init__(self, ext_path, det_path, rec_path,
        ...
        self.aligner = Aligner_PCA(is_aligned = is_aligned)
    ...
    def _forward_once(self, volume):
        ...
        # ------------------- merge -------------------
        regions, region_ptrs, region_map, region_pt_tuple, neuron_dict, (std, max_min) = self.merge_opt(volume, regions, scores, cur_num_region)
        # ------------------- neuron recognition -------------------
        merge_is_warning, _neuron_pt_tuple, neuron_dict, region_map, regions, region_ptrs, rec_is_warning, neuron_emb = self.rec_opt(region_pt_tuple, neuron_dict, region_map, regions, region_ptrs)
    ...
    def ext_opt(self, volume):
        if self.is_ext:
            ...
            h, w = min(self.det_input_dim[-2], volume.shape[-2]), min(self.det_input_dim[-1], volume.shape[-1])
            head_bboxes[:, :4] = torch.IntTensor([[w // 2, h // 2, w, h]]).repeat(volume.shape[0], 1)  # [cx, cy, w, h]
        return head_bboxes
    ...
    def merge_opt(self, volume, regions, scores, cur_num_region):
        ...
        # region point cloud alignment
        region_pt_tuple[:, :2], std, max_min = self.aligner(region_pt_tuple[:, :2])

        return regions, region_ptrs, region_map, region_pt_tuple, neuron_dict, (std, max_min)
    ...
    def rec_opt(self, region_pt_tuple, neuron_dict, region_map, regions, region_ptrs):
        ...
        neuron_pt_tuple[:, -1] = 0.  # the last channel is abandoned
        ...
        return merge_is_warning, _neuron_pt_tuple, neuron_dict, region_map, regions, region_ptrs, rec_is_warning, neuron_emb
...
if __name__ == '__main__':
    ...
    # config = "/home/cbmi/CBMI_python/src/configs/inference/wenlab_olfactory_si_womp.json"
    # config = "/home/cbmi/CBMI_python/src/configs/inference/wenlab_olfactory.json"
    config = "/home/wenlab-user/RongWei/olfactory/code_v1.10/src/configs/inference/240125.json"
    ...
    model = Treeformer_End2End(
        ...
        rec_keep_ratio = config["rec_keep_ratio"],

        is_aligned = config.get("is_aligned", True),

    ).eval().cuda().half()

    # benchmark(model, volume, 'fp16')
    buffer = VolumeMemoryBuffer(
        # save_path = "/home/cbmi/CBMI_python/data/zone_end20231230/cache2",
        deg_t = 100,
        weight_W = 0.2,
        warmup_num_vol = 3,
    )
    ...
    for path in [
        ...
        "/home/wenlab-user/RongWei/olfactory/20240113/w1/w1_2/gcamp/synthetic_volume/w1_2_aligned_volumes_all_mip.npy"
    ]:
        # path = "/home/cbmi/CBMI_python/data/temp/w2_aligned_volumes_part_mip.npy"
        # path = "/home/wenlab-user/RongWei/olfactory/20240113/w1/w1_2/gcamp/synthetic_volume/w1_2_aligned_volumes_all_mip.npy"
        ...
        with torch.inference_mode():
            ...
            # np.save('/home/wenlab-user/RongWei/olfactory/20240113/w1/w1_2/gcamp/synthetic_volume/w1_2_neuron_pt_tuple.npy', neuron_pt_tuple.cpu().numpy())
            ...
            print(f" the number of regions: {num_region}, \t the number of neurons: {num_neuron}, \t  the number of W: {num_neuron_idv}")
            # draw_volume_result(volume, head_bboxes, regions, region_pred_ids, region_ptrs, save_fig_root = "/home/wenlab-user/RongWei/olfactory/20240113/w1/w1_2/gcamp/synthetic_volume", name = "w1_2_aligned_volumes_mip", verbose = False)
