...
def auto_preprocess(mode: int, paths, args):
    ...
    :param mode: the flag of preproc
    ...
    params = [[stack_path, mode, args.zrange, "", args.save_preprocess_result_root, args.name_reg] for stack_path in paths]
    with Pool(min(8, len(params))) as p:
        with tqdm(p.imap_unordered(preprocess, params), total = len(params), desc = "Stack loading + S.1 preproc") as pbar:
