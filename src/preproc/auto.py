...
from src.comm_utils.dataset_building import get_stack_name
from src.comm_utils.prints import pad_num, print_warning_message, print_log_message
from src.preproc.anno2json import save_anno_as_json
from src.preproc.recognition_utils import calc_cylindrical_rho_phi, calc_mean_pt_of_3d_neuron, calc_mean_pt_of_bbox
...
# ----- General --------------------
def load_worm_mat(path: str, is_normalize: bool = False, z_range = (0, 18), pixel_range = (0, 3000), is_static = False):
    try:
        if is_static:
            stack = [np.stack(v[:, 0], axis = 0) for k, v in loadmat(path).items() if k.lower().startswith('img') or k.lower().startswith('image')][0]
        else:
        ...
        if is_static:
            stack = [np.stack([vol for vol in v]) for k, v in mat73.loadmat(path).items() if k.lower().startswith('img') or k.lower().startswith('image')][0]
        else:
            stack = [np.stack([vol[0][..., z_range[0]:z_range[1]] for vol in v]) for k, v in mat73.loadmat(path).items() if k.lower().startswith('img') or k.lower().startswith('image')][0]
    print_log_message(f"load {path}, which shape is {stack.shape}")
...
def get_head_contour(std_img, warning_head_area = 25000.0, is_convex: bool = False):
    ...
    # if area is bigger than warning_head_area, the area may the union of head and tail
...
# ----- pre-process --------------------
# ----- General -----
def save_image(path, image, _min = 0, _max = 1200):
    # np.save("/home/cbmi/CBMI_python/data/demos/mip.npy", image)
    plt.imsave(path, image, cmap = 'afmhot', vmin = 100, vmax = 400)
...
def preprocess(params):
    ...
    try:
        vols = load_worm_mat(stack_path, is_normalize = False, z_range = z_range, is_static = "static" in stack_path.split("/")[-1].split(".")[0].lower())
    ...
    for idx, volume in enumerate(vols):
        volume_name = stack_name + "_" + pad_num(idx, 6)
        # main step
        result = preprocess_volume(volume = volume, mode = mode, load_path = os.path.join(load_path, stack_name, f"{pad_num(idx, 6)}.json") if load_path != "" else load_path)
        results[volume_name] = result

        if mode == 4:
            save_image(os.path.join(result_path, f"{pad_num(idx, 6)}.png"), result[1])
        elif mode == 5:
            image_path = f"{pad_num(idx, 6)}.png"
            save_image(os.path.join(result_path, image_path), result[1])
            save_anno_as_json(mode = mode,
                              json_path = os.path.join(result_path, f"{pad_num(idx, 6)}.json"),
